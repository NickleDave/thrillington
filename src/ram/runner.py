""" "runner" for RAM model, from [1]_.
For recording behavior given specified initial starting locations.
...
class Runner:
    """Class for 'running' episodes using a trained model"""
    def __init__(self,
                 config,
                 batch_size,
                 learning_rate,
        ...
        self.logger.info(f'Number of samples in data: {self.num_samples}')
    ...
    def run(self, results_dir, save_examples=False, num_examples_to_save=None):
        """record behavior and compute accuracy of trained RAM models on a data set"""
        if not os.path.isdir(results_dir):
            raise NotADirectoryError(f"Couldn't find directory with results: {results_dir}")
        timenow = datetime.now().strftime('%y%m%d_%H%M%S')
        run_results_dir = os.path.join(results_dir, 'run_results_' + timenow)
        ...
        for replicate in range(1, self.replicates + 1):
            ...
            run_results_dir_this_replicate = os.path.join(run_results_dir, f'replicate_{replicate}')
            os.makedirs(run_results_dir_this_replicate)
            if save_examples:
                examples_dir = os.path.join(run_results_dir_this_replicate, 'examples')
            ...
            (accs,
             preds,
             true_lbl,
             sample_inds) = self._run_one_model(save_examples=save_examples,
            ...
            for name, arr in zip(['accs', 'preds', 'true_lbl', 'sample_inds'],
                ...
                fname = os.path.join(run_results_dir_this_replicate, name + '.npy')
                np.save(fname, arr)

            self.logger.info(f'mean accuracy on data set: {np.mean(accs)}')

    def _run_one_model(self, save_examples, num_examples_to_save, examples_dir):
        accs = []
        sample_inds = []
        preds = []
        true_lbl = []
        ...
        with tqdm(total=total) as progress_bar:
            batch = 0
            for img, lbl, batch_sample_inds in self.data.batch(self.batch_size):
                ...
                for ep in range(self.num_mc_episode):
                    ...
                    if self.l0 is not None:
                        l_t = np.broadcast_to(self.l0, shape=(self.batch_size, 2))
